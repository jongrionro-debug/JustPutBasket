...
"caption", "category", "silhouette", "color", "material", "pattern", "texture", "mood", "season", "era", "detail", "review_needed", "confidence_note"
...
- silhouette: one concise silhouette label for the overall look
- color: dominant main color only
- material: main visible material only
- pattern: main visible pattern only
- texture: main visible texture only
- mood: one concise fashion mood label
- season: one likely season label if visually evident
- era: one style-era label if visually evident
...
Output example:
{"caption":"black wool coat with wide-leg trousers on runway","category":["coat","trousers","shoes"],"silhouette":"tailored","color":"black","material":"wool","pattern":"","texture":"smooth","mood":"minimal","season":"fall","era":"","detail":["long wool coat","wide-leg trousers","heels"],"review_needed":false,"confidence_note":"high confidence"}
""".strip()
...
def _coerce_json(
    ...
    normalized = {
        ...
        **{
            feature: (
                _coerce_multi_value(payload.get(feature, ""))
                if feature in {"category", "detail"}
                else str(payload.get(feature, "")).strip()
            )
            for feature in DEFAULT_ATTRIBUTE_NAMES
            if feature != "category"
        },
...
def _write_raw_output_log(
    ...
    stem = Path(image_path).stem if image_path else "model-output"
    digest = hashlib.sha1(raw_text.encode("utf-8")).hexdigest()[:10]
    destination = log_dir / f"{stem}-{digest}-{status}.txt"
