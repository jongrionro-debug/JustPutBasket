...
이번 V1에서 OpenAI API 비용은 거의 `생성 이미지 비용`뿐이다.
...
- archive image embedding: local CLIP inference

- query text embedding: local CLIP text encoder

- generated image embedding: local CLIP image encoder

  

즉 OpenAI 비용이 붙는 지점은 `reference image generation`뿐이다.

  

### archive embedding 비용

  

- OpenAI API 비용: `$0`

- 대신 드는 것: 로컬 추론 시간, 메모리, 저장 공간

  

이건 billing cost가 아니라 compute cost다.

  

### query text embedding 비용
...
- query text도 local CLIP text encoder로 처리하기 때문
...
query `10개`, query당 `1장` 생성:

  

- `gpt-image-1-mini medium`: 약 `$0.11`

- `gpt-image-1.5 medium`: 약 `$0.34`

  

query `10개`, query당 `4장` 생성:

  

- `gpt-image-1-mini medium`: 약 `$0.44`

- `gpt-image-1.5 medium`: 약 `$1.36`

  

즉 현재 실험 규모에서 OpenAI 비용은 archive 크기보다 `query 수 x 생성 이미지 수`에 의해 결정된다.
